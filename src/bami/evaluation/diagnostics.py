...
DIAGNOSTIC_SAMPLE_BATCH_SIZE = 16
...
def _as_param_list(params: str | Sequence[str] | None) -> list[str] | None:
    ...
    -------
    list[str] | None
        Normalized parameter list, or ``None`` when all available parameters
        should be used.
    """
...
def _as_metric_list(metrics: str | Sequence[str]) -> list[str]:
    ...
    -------
    list[str]
        Validated metric names in the requested order.
    """
...
def _posterior_mean(samples: Mapping[str, np.ndarray], key: str) -> np.ndarray:
    ...
    -------
    numpy.ndarray
        Posterior mean array with the sample axis removed.
    """
...
def _finite_pairs(truth, estimate) -> tuple[np.ndarray, np.ndarray]:
    ...
    ----------
    truth, estimate
        Arrays with matching shapes after posterior averaging.

    Returns
    -------
    tuple[numpy.ndarray, numpy.ndarray]
        One-dimensional finite truth and estimate vectors.
    """
...
def _compute_metric(metric: str, truth: np.ndarray, estimate: np.ndarray) -> float:
    ...
    truth, estimate
        Simulated truth and posterior means.
...
def _compute_random_metric(
    ...
    metric
        Metric name, one of ``corr``, ``ccc``, or ``rmse``.
    truth, estimate
        Subject-level simulated truth and posterior means for one dataset.
...
def _format_title(
    ...
    *,
    ...
    truth, estimate
        Paired one-dimensional recovery values.
    metrics
...
def _plot_random_metric_distribution(
    ...
    *,
    ...
    -------
    matplotlib.figure.Figure
        Random recovery metric distribution figure.
    """
...
def _simple_recovery_params(model, simulated_data, samples, params) -> list[str]:
    ...
    simulated_data, samples
        Simulated truth dictionary and posterior sample dictionary.
    params
...
def _population_recovery_params(model, simulated_data, samples, params) -> list[str]:
    ...
    simulated_data, samples
        Simulated truth dictionary and group posterior sample dictionary.
    params
        Optional requested parameter names.
    ...
    -------
    list[str]
        Public group-level parameter keys to plot.
    """
...
def _random_recovery_params(model, simulated_data, samples, params) -> list[str]:
    ...
    model
        ``HierarchicalWorkflow`` instance.
    simulated_data, samples
        Simulated truth dictionary and random posterior sample dictionary.
    params
        Optional requested parameter names.
    ...
    -------
    list[str]
        Subject-level public parameter names to plot.
    """
...
def plot_parameter_recovery(
    ...
    *,
    ...
    ----------
    model
        ``SimpleWorkflow`` instance used to simulate datasets and sample the
        fitted posterior.
    n_datasets
    ...
    params
        ...
        inferred parameters with both simulated truth and posterior samples are
        shown.
    metrics
        Metric name or names shown in each panel title. Supported values are
        ``corr``, ``ccc``, and ``rmse``.
    n_cols
    ...
    samples = model.sample_posterior(
        ...
        sample_batch_size=DIAGNOSTIC_SAMPLE_BATCH_SIZE,
    )
...
def plot_population_recovery(
    ...
    *,
    ...
    ----------
    model
        ``HierarchicalWorkflow`` instance used to simulate group datasets and
        sample group-level posteriors.
    n_datasets
        Number of simulated group datasets used for the diagnostic plot.
    num_samples
        Number of group posterior draws per simulated dataset.
    params
        Optional population parameter key or keys to plot, such as ``"c_mu"``
        or ``"c_sigma"``. By default all available public group keys are shown.
    metrics
        Metric name or names shown in each panel title. Supported values are
        ``corr``, ``ccc``, and ``rmse``.
    n_cols
...
def plot_random_recovery(
    ...
    *,
    ...
    model
        ``HierarchicalWorkflow`` instance used to simulate group datasets and
        sample paired group and random posteriors.
    n_datasets
        Number of simulated group datasets used for the diagnostic plot.
    num_samples
        Number of group posterior draws per simulated dataset.
    params
        Optional subject-level parameter name or names to plot, such as ``"c"``
        or ``"kappa"``. By default all saved subject-truth parameters with
        posterior samples are shown.
    metrics
        Metric name or names to plot. Supported values are ``corr``, ``ccc``,
        and ``rmse``. Each metric is shown in a separate panel.
    n_cols
    ...
    names = _random_recovery_params(model, simulated_data, random_samples, params)
    rows = []
    for name in names:
        truth_key = f"{name}_subj"
